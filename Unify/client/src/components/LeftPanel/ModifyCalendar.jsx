...
export const ModifyCalendar = ({ calendarid, accountid, onClose, onSave }) => {
    ...
    useEffect(() => {
        if (calendarid) {
            calendarService.getCalendar(calendarid)
                ...
                .catch(err => setErrors("Calendar not found or error loading calendar."));
        }
    }, [calendarid]);
    ...
    useEffect(() => {
        ...
        setCalendarDescription(calendarData.calendardescription || "")
        setCalendarPrivacy(calendarData.calendarprivacy || "public")
        setCalendarColour(calendarData.calendarcolour || "#90D5FF")
    }, [calendarData])
    ...
    const handleDelete = async (e) => {
        try {
            const res = await calendarService.deleteCalendar({ calendarid : calendarData.calendarid})
            if (onClose) {
                onClose()
            }
        }
    ...
    const handleSave = async (e) => {
        ...
        try {
            const res = await calendarService.modifyCalendar({
            ...
            setErrors(["Failed to modify calendar. Please try again."])
        }
    }
